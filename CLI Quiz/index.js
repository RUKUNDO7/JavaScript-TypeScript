...
async function main() {
    ...
    p.intro(`${pc.bgMagenta(pc.black('Welcome. Let us find out how much of a CLI expert you are!'))}`);
    ...
    const question2 = new Question(
        "2) What was the display technology used in early computer terminals that employed glowing green characters on a dark background?",
        [
	...
	if (readyToPlay == "Yes") {
		...
		p.outro(`${pc.bgMagenta(pc.black(`You got ${totalCorrect} questions correct!`))}`);
		...
		if (totalCorrect == 10) {
			...
			s.stop();
			p.outro(`${pc.bgMagenta(pc.black(`The command line is a tool that is ripe for change. `))}`);
		} else {
			const s = p.spinner();
            s.start();
			await setTimeout(3000);
			s.stop();
			p.outro(`${pc.bgMagenta(pc.black(`You need 10/10 correct to unlock the secret message. Try again.`))}`);
		}
	} else {
		p.outro(`${pc.bgMagenta(pc.black(`Ok. Bye!`))}`);
	}
